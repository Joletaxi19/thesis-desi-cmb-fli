...
from jax import jit, tree, tree_util
from jax import numpy as jnp
from jax import random as jr
from numpyro.diagnostics import print_summary

from desi_cmb_fli.metrics import multi_ess
from desi_cmb_fli.utils import nvmap
    ...
    def stackby(self, names: str | list = None, remove=True, axis=-1):
        ...
        for k in names:
            if k not in self:  # if name is a variable do noting
                ...
                if remove:
                    for var_name in self.groups[k]:
                        new.data.pop(var_name)
        return new
