...
from desi_cmb_fli.bdec import credint
...
def plot_bivar(fn, box=None, n=50, type="mesh", **kwargs):
    """
...
def plot_mesh(
    mesh,
    box_shape=None,
    sli: int | float | slice = None,
    axis=-1,
    vlim: float | tuple[float, float] = 1e-4,
    **kwargs,
):
    """
...
def anim_meshes(
    meshes, box_shape=None, vlim: float | tuple[float, float] = 1e-4, cmap="viridis", pause=10
):
    """
    ...
    anim = animation.FuncAnimation(
        plt.gcf(), update, frames=len(meshes) + pause, interval=100, blit=True
    )
    plt.close()
    display(anim)
...
def plot_powtranscoh(
    ks, pow1, trans, coh, *args, log=False, fill: float = None, axes: list = None, **kwargs
):
    outs = []
